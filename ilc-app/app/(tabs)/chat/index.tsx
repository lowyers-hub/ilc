...
const SUGGESTIONS = [
  "Saya di-PHK mendadak, apa hak saya?",
  "Kontrak saya dibatalkan sepihak, apa yang bisa saya lakukan?",
  "Saya dipanggil polisi, harus bagaimana?",
  "Utang saya tidak dibayar, langkah hukum apa yang bisa saya ambil?"
];
...
export default function ChatHomeScreen() {
  ...
  useEffect(() => {
    logEvent('app_opened', { timestamp: Date.now() });
  }, []);
  ...
  const handleCreateSession = async (initialMessage?: string) => {
    ...
    logEvent('chat_started', { sessionId: s.id, timestamp: Date.now() });
            ...
            {sessions.isLoading ? (
              ...
              <View className="items-center w-full max-w-md">
                ...
                <Text className="text-subtext text-base text-center mb-8 px-4 leading-relaxed">
                  ...
                  {'\n\n'}
                  <Text className="text-danger font-semibold">Perhatian:</Text> Ini bukan nasihat hukum final.
                </Text>
                
                <View className="w-full gap-3">
                  <Text className="text-subtext text-sm font-semibold mb-1 uppercase tracking-wider">Contoh Pertanyaan:</Text>
                  {SUGGESTIONS.map((suggestion, idx) => (
                    ...
                    >
                      <Text className="text-text text-[15px]">{suggestion}</Text>
                    </Pressable>
                  ))}
                </View>
              </View>
